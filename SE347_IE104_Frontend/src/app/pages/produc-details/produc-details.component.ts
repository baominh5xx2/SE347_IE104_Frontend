...
@Component({
  ...
  imports: [CommonModule, TourCardComponent, ToastModule],
...
export class ProducDetailsComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.authStateService.isAuthenticated$.subscribe(isAuth => {
      ...
      if (isAuth && this.relatedTours.length > 0) {
  ...
  async loadRelatedTours(): Promise<void> {
    ...
    try {
      ...
      if (response && response.packages && Array.isArray(response.packages)) {
        ...
        if (this.isAuthenticated && this.relatedTours.length > 0) {
          this.loadFavoriteStatuses();
        }
      }
    } catch (error) {
  ...
  async loadFavoriteStatuses(): Promise<void> {
    ...
    const promises = this.relatedTours.map(async (tour) => {
      try {
  ...
  getInfoCards(): Array<{type: string, icon: string, label: string, value: string}> {
    ...
    // 2. Ngày khởi hành (always second if available)
    ...
    // 3. Còn chỗ (always third if available)
    if (this.tour.available_slots !== undefined && this.tour.available_slots !== null) {
      cards.push({
        type: 'slots',
        icon: 'fas fa-users',
        label: 'CÒN CHỖ',
    ...
    // Optional: Xuất phát (if no other optional cards)
    if (this.tour.departure_location && cards.length < 5) {
      cards.push({
        type: 'departure',
    ...
    // Optional: Giảm giá (if no other optional cards)
    if (this.tour.discount && cards.length < 5) {
      cards.push({
        type: 'discount',
        icon: 'fas fa-tag',
        label: 'GIẢM GIÁ',
